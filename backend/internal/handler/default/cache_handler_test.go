...
func (s *CacheHandlerTestSuite) TestGetStatus_Success() {
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)

	s.mockCache.EXPECT().Status().Return("connected")

	s.handler.GetStatus(c)
	...
	assert.Equal(s.T(), "connected", data["status"])
}
